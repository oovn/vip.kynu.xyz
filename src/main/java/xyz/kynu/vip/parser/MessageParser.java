...
import xyz.kynu.vip.crypto.axolotl.NotEncryptedForThisDeviceException;
...
import xyz.kynu.vip.entities.ReceiptRequest;
...
public class MessageParser extends AbstractParser implements OnMessagePacketReceived {
	...
	public void onMessagePacketReceived(Account account, MessagePacket original) {
		...
		if ((body != null || (axolotlEncrypted != null && axolotlEncrypted.hasChild("payload")) || oobUrl != null || xP1S3 != null) && !isMucStatusMessage) {
			...
			if (axolotlEncrypted != null && Config.supportOmemo()) {
				Jid origin;
				...
				message = new Message(conversation, oobUrl, Message.ENCRYPTION_AXOLOTL, status);
				message.setOob(true);
			} else {
				message = new Message(conversation, body, Message.ENCRYPTION_AXOLOTL, status);
			}
