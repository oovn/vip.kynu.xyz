...
public class Conversation extends AbstractEntity implements Blockable, Comparable<Conversation>, Conversational {
	...
	public void findMessagesWithFiles(final OnMessageFound onMessageFound) {
		...
		synchronized (this.messages) {
			for (final Message m : this.messages) {
				if (m.isFileOrImage() && m.getEncryption() == Message.ENCRYPTION_AXOLOTL) {
					results.add(m);
				}
			}
		}
	...
	public Message getLatestMessage() {
		synchronized (this.messages) {
			if (this.messages.size() == 0) {
				Message message = new Message(this, "", Message.ENCRYPTION_AXOLOTL);
	...
	public int getNextEncryption() {
		if (OmemoSetting.isAlways()) {
			return suitableForOmemoByDefault(this) ? Message.ENCRYPTION_AXOLOTL : Message.ENCRYPTION_AXOLOTL;
		}
